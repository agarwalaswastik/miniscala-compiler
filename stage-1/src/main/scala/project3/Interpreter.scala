...
class StackInterpreter extends Interpreter with BugReporter {
  ...
  def evalBin(op: String)(sp: Loc, sp1: Loc) = (op, memory(sp), memory(sp1)) match {
    ...
    case _    =>
      BUG(s"Binary operator $op undefined")
  }
  ...
  def eval(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case LetRec(funs, body) =>
      val funcs = funs.zipWithIndex map {
        case (fun@FunDef(name, _, _, _), i) =>
          eval(fun, sp + i)(env)
          (name, sp + i)
      }

      funcs.foreach { case (_, location) =>
        memory(location) match {
          case func@Func(_, _, _) => func.withVals(funcs)
        }
      }
      
      eval(body, sp)(env.withVals(funcs))
    case App(fun, args) =>
      eval(fun, sp)(env)
      ...
      memory(sp) match {
        case Func(fargs, fbody, fenv) =>
          val argLocations = (0 until args.length).map(i => sp + 1 + i)
          eval(fbody, sp + args.length + 1)(fenv.withVals(fargs.zip(argLocations)))
          memory(sp) = memory(sp + args.length + 1)
        case Primitive("getchar") =>
          memory(sp) = Cst(Console.in.read)
        case Primitive("putchar") =>
          val Cst(c: Int) = memory(sp + 1)
          Console.out.write(c)
          Console.out.flush
          memory(sp) = Cst(())
        case _ =>
