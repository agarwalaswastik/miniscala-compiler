...
object CPSValueRepresenter extends (H.Tree => L.Tree) {
  ...
  private def transform(tree: H.Tree)
    ...
    */
    ...
    // *************** Block Primitives *********************** -- done
    case H.LetP(name, MiniScalaBlockAlloc(k), Seq(s), body) =>
      tempLetL(1) { c1 =>
        tempLetP(CPSArithShiftR, Seq(s, c1)) { us =>
          L.LetP(name, CPSBlockAlloc(k), Seq(us), transform(body))
        }
      }
    ...
    */
    ...
    // *************** Function Nodes *********************** -- done
    // Optimized version with advanced closure conversion
    case H.LetF(funs, body) if optimized =>
      val funNames = funs map (_.name)

      // *************** Workers ***************
      // Compute free variables for each function
      val workerFVs = {
        ...
        fixedPoint(Map(funNames map { n => (n, Set.empty[Symbol]) } :_*)) {
          funFVsApprox =>
            ...
            Map(funs map { f =>
              (f.name, freeVariables(f.body)(allFVs) -- f.args)
            } : _*)
        }
      }

      // Ordered free variables for each function
      val workerLinFVs = workerFVs map { case (f, fv) =>
        (f, if (fv(f)) f +: ((fv - f).toSeq) else fv.toSeq)
      }

      // Generate fresh worker names
      val workerName =
        (funNames map { n => (n, Symbol.fresh(n.toString)) }).toMap

      // New map of available workers for nested functions
      val newWorker = worker ++ (
        workerLinFVs map { case (n, fvs) => (n, (workerName(n), fvs)) })

      // Create worker functions with environment handling
      ...
      // *************** Compute Escaping Functions ***************
      // We only create wrappers for functions that escape their scope
      ...
      // *************** Wrappers ***************
      // Ordered free variables for wrappers (excluding self-reference)
      val wrapperFVs = workerLinFVs map { case (f, fv) =>
        (f, fv filter (_ != f))
      }

      // Build wrapper functions for escaping functions
    ...
    // Non-optimized version with simple closure conversion
    case H.LetF(funs, body) => // if !optimized =>
      val funNames = funs map(_.name)
    ...
    // Optimized function application using worker map
    ...
    // Standard function application through block lookup
  ...
  /**
   * Utility function to compute fixed point for free variable analysis.
   * Used in optimized function handling to determine closure requirements.
   */
  private def fixedPoint[T](start: T)(f: T => T): T = {
    def iterate(current: T): T = {
      val next = f(current)
      if (next == current) current else iterate(next)
    }
    iterate(start)
  }
}